...
export default function DailyReward() {
  const [claimed, setC] = useState(useStore.getState().dailyClaimed)
  const id = useStore(s => s.activeChildId); const upd = useStore(s => s.updateChild)
  const handle = () => {
    if (!id || claimed) return
    if (db.claimDaily(id)) {
      setC(true); useStore.getState().setDailyClaimed(true)
      const c = useStore.getState().children.find(x => x.id === id)
      if (c) upd(id, { foodBalance: c.foodBalance + 50, lastDailyRewardAt: Date.now() })
    }
  }
  return <div className="reward-card">
