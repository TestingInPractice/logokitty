  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <header className="header">
        ...
        {path.startsWith('/child') && <button className="btn btn-sm btn-outline" onClick={() => n('/feed')}>К родителю</button>}
      </header>
      ...
      <nav style={{ display: 'flex', background: '#fff', borderTop: '1px solid #f0e8e0', padding: '8px 0', paddingBottom: 'max(8px,env(safe-area-inset-bottom))' }}>
        {tabs.map(t => (
          <button key={t.path} onClick={() => { if (t.path === '/child' && !id) return; n(t.path) }}
            style={{
              flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '4px 0', background: 'transparent', fontSize: 12,
              fontWeight: path.startsWith(t.path) ? 700 : 400,
              color: path.startsWith(t.path) ? 'var(--orange-dark)' : 'var(--text-light)',
